...
class Trainer:
    ...
    def __init__(self, GPU, data_dir, fixed_dir, fonts_num, batch_size, img_size):
        ...
        import pickle
#       with open(fixed_dir + '/EMBEDDINGS.pkl', 'rb') as rf:
        with open(os.path.join(fixed_dir, 'EMBEDDINGS.pkl'), 'rb') as rf:
            self.embeddings = pickle.load(rf)
#       self.embeddings = torch.load(os.path.join(fixed_dir, 'EMBEDDINGS.pkl'))
#       self.embeddings = init_embedding(25, 128)
        self.embedding_num = self.embeddings.shape[0]
        self.embedding_dim = self.embeddings.shape[3]
        
        self.fixed_source = torch.load(os.path.join(fixed_dir, 'fixed_source.pkl'), map_location=torch.device('cpu'))
        self.fixed_target = torch.load(os.path.join(fixed_dir, 'fixed_target.pkl'), map_location=torch.device('cpu'))
        self.fixed_label = torch.load(os.path.join(fixed_dir, 'fixed_label.pkl'), map_location=torch.device('cpu'))
    ...
    def train(self, max_epoch, schedule, save_path, to_model_path, lr=0.001, \
        ...
        if not fine_tune:
            L1_penalty, Lconst_penalty = 100, 0# 100, 15
        else:
            L1_penalty, Lconst_penalty = 0, 1000 # 500, 1000
        ...
        if self.GPU:
            l1_criterion = nn.L1Loss(size_average=True).to(self.GPU)
            bce_criterion = nn.BCEWithLogitsLoss(size_average=True).to(self.GPU)
            mse_criterion = nn.MSELoss(size_average=True).to(self.GPU)
        else:
        ...
        for epoch in range(max_epoch):
            ...
            for i, batch in enumerate(train_batch_iter):
                ...
                if self.GPU:
                    batch_images = batch_images.to(self.GPU)
                if flip_labels:
...
#                   real_target[idx] = torch.tensor(image_T).view([1, self.img_size, self.img_size])
                ...
                if self.GPU:
                    real_category = real_category.to(self.GPU)
                ...
                # labels
                if self.GPU:
                    one_labels = torch.ones([self.batch_size, 1]).to(self.GPU)
                    zero_labels = torch.zeros([self.batch_size, 1]).to(self.GPU)
                else:
...
def interpolation(data_provider, grids, fixed_char_ids, interpolated_font_ids, embeddings, \
    ...
    for grid_idx, grid in enumerate(grids):
        ...
        for i, batch in enumerate(train_batch_iter):
            ...
            interpolated_embeddings = torch.tensor(interpolated_embeddings).to(self.GPU)
