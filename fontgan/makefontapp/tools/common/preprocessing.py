...
from .pilutil import imresize # scipy.__version__ == 1.2.0
...
def cropping(img_arr):
    ...
    if img_arr[0][0] != 255:
        ...
        color = 255
    full_white = np.asarray(Image.new('L', (img_arr.shape[1], img_arr.shape[0]), color=color)).astype(np.float)
    col_contrast = (np.sum(full_white, axis=0) - np.sum(img_arr, axis=0))
    row_contrast = (np.sum(full_white, axis=1) - np.sum(img_arr, axis=1))
    col_sum = np.where(col_contrast > 1)
    row_sum = np.where(row_contrast > 1)
