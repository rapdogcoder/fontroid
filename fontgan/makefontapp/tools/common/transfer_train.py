...
class Trainer:
    ...
    def train(self, max_epoch, schedule, save_path, to_model_path, lr=0.001, \
        ...
        if len(restore) == 3:
            ...
            En.load_state_dict(torch.load(os.path.join(from_model_path, encoder_path), map_location=torch.device('cpu')))
            De.load_state_dict(torch.load(os.path.join(from_model_path, decoder_path), map_location=torch.device('cpu')))
            D.load_state_dict(torch.load(os.path.join(from_model_path, discriminator_path), map_location=torch.device('cpu')))
