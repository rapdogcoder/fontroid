...
trg_font_path = os.path.join(SCRIPT_PATH, '../data/trg_font')
...
def create_model(src_font, trg_font, trg_skeleton, style_labels, args):
    ...
    with tf.name_scope("f2s_generator"):
        with tf.variable_scope("generator", reuse=True):
            out_channels = int(trg_font.get_shape()[-1])
            f2s_outputs = create_generator(f2f_outputs, out_channels, style_labels, args)
    ...
    with tf.name_scope("f2s_real_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 7, 7, 1]
            f2s_predict_real, f2s_real_styl = create_discriminator(f2f_outputs, trg_skeleton, args)

    with tf.name_scope("f2s_fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 7, 7, 1]
            f2s_predict_fake, f2s_fake_styl = create_discriminator(f2f_outputs, f2s_outputs, args)
    ...
    with tf.name_scope("s2f_generator"):
        with tf.variable_scope("generator", reuse=True):
            out_channels = int(trg_font.get_shape()[-1])
            s2f_outputs = create_generator(f2s_outputs, out_channels, style_labels, args)
    ...
    with tf.name_scope("s2f_real_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 7, 7, 1]
            s2f_predict_real, s2f_real_styl = create_discriminator(f2s_outputs, trg_font, args)

    with tf.name_scope("s2f_fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 7, 7, 1]
            s2f_predict_fake, s2f_fake_styl = create_discriminator(f2s_outputs, s2f_outputs, args)
