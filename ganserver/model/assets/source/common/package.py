...
def pickle_examples(from_dir, font_label, charids, train_path, val_path, train_val_split=0.2, with_charid=False):
    """
    ...
    with open(train_path, 'wb') as ft:
        with open(val_path, 'wb') as fv:
            ...
            if with_charid:
                ...
                for p in paths:
                    c += 1
                    label = font_label.index(os.path.basename(p).split("_")[0])
                    charid = charids.index(os.path.basename(p).split("_")[1].split(".")[0])
                    with open(p, 'rb') as f:
                        img_bytes = f.read()
